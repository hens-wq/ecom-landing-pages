...
function TableHead({ className, ...props }: React.ComponentProps<"th">) {
  return (
    <th
      ...
      className={cn(
        "h-10 whitespace-nowrap px-3 text-right align-middle text-xs font-medium text-muted-foreground [&:has([role=checkbox])]:pl-0",
        className
      )}
      {...props}
    />
...
function TableCell({ className, ...props }: React.ComponentProps<"td">) {
  return (
    <td
      ...
      className={cn("whitespace-nowrap px-3 py-2.5 align-middle [&:has([role=checkbox])]:pl-0", className)}
      {...props}
    />
  );
}
