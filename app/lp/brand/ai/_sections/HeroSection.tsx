...
/**
 * Structural clone of Cyber's HeroSection — see that file for the layout
 * rationale. Only the section background token (ai-dark instead of
 * ink-950) and the CTA accent (teal instead of brand) differ; the
 * turquoise badge/lime salary figure already matched the AI palette
 * as-is and are unchanged.
 */
export function HeroSection() {
  const { hero } = aiContent;
  const { values, errors, status, submitError, tracking, updateField, handleSubmit } =
    useLeadForm("brand-ai");

  return (
    <section className="relative flex flex-col overflow-hidden bg-ai-dark lg:flex-row lg:min-h-[94vh]">
      <div className="absolute inset-x-0 top-0 h-[54svh] lg:inset-0 lg:h-auto">
        ...
        />
        <div className="absolute inset-0 bg-gradient-to-t from-ai-dark from-[4%] via-ai-dark/45 via-[42%] to-transparent to-[75%] lg:hidden" />
        <div className="absolute inset-0 hidden bg-gradient-to-t from-ai-dark/70 via-ai-dark/10 to-transparent lg:block" />
        <div className="absolute inset-0 hidden bg-gradient-to-l from-ai-dark/95 via-ai-dark/10 to-transparent lg:block" />
      </div>
      ...
      >
        <span className="text-lg font-bold text-white drop-shadow-[0_2px_10px_rgba(0,0,0,0.5)] sm:text-xl">
          מכללת Ecom
        </span>
      </motion.div>
      ...
      >
        ...
        >
          <CTAButton href="#lead-form-hero" size="lg" accent="teal">
            {hero.ctaLabel}
          </CTAButton>
        </motion.div>
      </div>
      ...
      <div className="relative z-10 lg:absolute lg:inset-y-0 lg:end-0 lg:flex lg:w-[400px] lg:items-center xl:w-[440px]">
        <div
          id="lead-form-hero"
          className="border-t border-white/10 bg-ai-dark/85 px-5 py-8 backdrop-blur-md sm:px-8 md:px-10 lg:w-full lg:border-t-0 lg:border-s lg:border-white/10 lg:py-10 lg:[clip-path:polygon(0%_0%,92%_0%,100%_100%,0%_100%)]"
        >
          <AnimatePresence mode="wait">
            {status === "success" ? (
              ...
              >
                ...
                <CTAButton type="submit" fullWidth loading={status === "submitting"} accent="teal">
                  {hero.ctaLabel}
                </CTAButton>
              </motion.form>
            )}
