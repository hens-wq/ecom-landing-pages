...
export function CyberExperienceSection() {
  const { cyberExperience } = cyberContent;
  ...
  return (
    <section className="relative overflow-hidden bg-ink-950 px-5 py-16 sm:px-8 md:px-16 md:py-24">
      ...
      />
      <div
        className="pointer-events-none absolute -top-32 start-1/4 size-[520px] rounded-full bg-brand-600/20 blur-[120px]"
        aria-hidden
      />
      <div
        className="pointer-events-none absolute -bottom-24 end-1/4 size-[420px] rounded-full bg-teal-500/15 blur-[110px]"
        aria-hidden
      />
        ...
        <div className="relative min-h-[560px]">
          ...
          {topics.map((topic, index) => {
            ...
            return (
              ...
              >
                <span className="inline-flex items-center gap-2 rounded-full border border-white/10 bg-ink-900/80 px-3.5 py-2 text-sm text-ink-100 backdrop-blur-sm">
        ...
        <div className="no-scrollbar mt-6 flex snap-x gap-2.5 overflow-x-auto pb-2">
          {topics.map((topic) => {
            ...
            return (
              <span
                key={topic.label}
                className="inline-flex shrink-0 snap-start items-center gap-2 rounded-full border border-white/10 bg-ink-900/80 px-3.5 py-2 text-sm whitespace-nowrap text-ink-100"
              >
                <Icon className="size-4 text-teal-400" aria-hidden />
                {topic.label}
              </span>
